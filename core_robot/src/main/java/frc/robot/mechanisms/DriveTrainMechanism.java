...
public class DriveTrainMechanism implements IMechanism
{
        ...
        ITimer timer)
    {
        ...
        for (int i = 0; i < DriveTrainMechanism.NUM_MODULES; i++)
        {
            ...
            this.driveMotors[i].setMotorOutputSettings(driveMotorInvertOutputs[i], MotorNeutralMode.Brake);
            this.driveMotors[i].setFeedbackUpdateRate(DriveTrainMechanism.FRAME_PERIOD_HZ);
            this.driveMotors[i].setErrorUpdateRate(DriveTrainMechanism.FRAME_PERIOD_HZ);
            ...
            this.steerMotors[i].setMotorOutputSettings(steerMotorInvertOutputs[i], MotorNeutralMode.Brake);
    ...
    public void readSensors()
    {
        for (int i = 0; i < DriveTrainMechanism.NUM_MODULES; i++)
        {
            ...
            this.steerAngles[i] = Helpers.updateAngleRange180(this.steerPositions[i] * HardwareConstants.DRIVETRAIN_STEER_PULSE_DISTANCE);
            this.steerErrors[i] = this.steerMotors[i].getError();
            this.encoderVoltages[i] = this.absoluteEncoders[i].getVoltage();
            this.encoderAngles[i] = Helpers.updateAngleRange180(this.encoderVoltages[i] * HardwareConstants.DRIVETRAIN_ENCODER_DEGREES_PER_VOLT);
    ...
    public void update(RobotMode mode)
    {
        ...
        for (int i = 0; i < DriveTrainMechanism.NUM_MODULES; i++)
        {
            ...
            Optional<TalonFXControlMode> driveControlMode = Optional.empty();
            ...
            if (driveVelocitySetpoint != null)
            {
                ...
                driveControlMode = Optional.of(TalonFXControlMode.Velocity);
            ...
            else if (drivePositionSetpoint != null)
            {
                ...
                driveControlMode = Optional.of(TalonFXControlMode.Position);
            ...
            if (driveControlMode.isPresent())
            {
                this.driveMotors[i].setControlMode(driveControlMode.get());
